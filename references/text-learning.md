...
任何诊断或教学前先完成[数据模式三选一](learning-entry.md)，不把选择兴趣节点当成允许读取画像或开始知识测试。选择后再确认当前目标的必要知识边界：创建模式问最小诊断，既有库核验已有证据并只补缺口，无个人数据模式只就当前问题临时确认前提。诊断题应区分不同的后续讲解路径，不能只问与目标无关、答案已在题干里的常识。

本协议的文字表达、新词落地、主动作答、局部反馈和独立检查原则适用于三种模式。下文所有 Vault 签发、过程回写、响应画像和保持排期规定，仅用于获准持久化分支；只读/无数据分支用会话内闭环，不运行 writer、不建立后台影子记录、不声称通过持久化验证。小任务可用纯文字对话，用户未要求时不为凑流程额外落文件。模式切换后废弃旧个人简报与待提交草稿，不能继续使用已撤回范围的锚点。
...
当前版本的持久化教学签发先读取 `prepare-teaching` 简报。`issue-teaching` 必须先确认 `routing_action=teach_now`；若为 `diagnose_now`，只发已绑定探针，提交诊断并重新签发合格学习步骤后才能教学。其他动作也不能绕过此门。内容 JSON 声明 `teaching_basis`：`anchor_ids` 只用已验证锚点 ID（没有就 `[]`），`focus_capabilities` 为本单元覆盖的当前合同能力非空子集。兴趣和熟悉题材只能作为情境，不能放进 anchor_ids 冒充理解。必要对比若无掌握证据，需要局部解释；不得把其名称直接用作定义。

输入使用[教学模板](../templates/teaching-content.json)，另外必须原样绑定简报中的 `route_binding_id / decision_fingerprint / brief_fingerprint`。简报来自实际已落地的决策，不预览一个尚未执行的新方法；`routing_action` 决定先诊断还是教学，`process_next_action` 只是过程反馈。新证据、目标或路线变化后先重新准备并审阅讲解；想换活动则先 `resolve-teaching`，不能仅替换指纹继续用旧草稿。

简报不含原始验证题和保护答案。生成器只读简报中的 opaque guard，不能为了准备讲解而加载完整 resource。已有记录不迁移；`issue-teaching` 对新输入强制此门。v0.1.4 在绑定检查外增加实际正文覆盖检查：`concept_inventory` 来自当前概念和本步邻近关系的名称/别名，不依赖作者是否申报新词。词表只用于查漏；未在本单元使用的邻近概念不强制教学，也不自动加进路径。
...
路线已指定的新词保留在 `introduced_terms` / 简报 `terms_to_ground`，用户内容为每项提供 `term + what_it_is + owner_scope + role_here + relation_direction`。v0.1.4 拟稿时发现的新承重词可直接补进 `term_grounding`，新签发用“必需词＋草稿新增词”做完整覆盖检查，不为措辞补充伪造新路线或教学方法时点。必需词必须按原名提供定义；名称/别名的对齐用于正文及定义依赖检查，不允许只换别名省掉必需项。普通日常词和未被使用的邻近词不扩成词典。

投影顺序也是依赖顺序：定义中引用另一个尚未解释的词时，先重排或改写；登记词的别名同样检查，不以漏报 introduced_terms 绕过。四项含义拒绝独立的“不知道”“待解释”“TODO”等占位，`what_it_is` 拒绝“聚焦圆锥就是聚焦圆锥”一类明确循环自称；不靠最小字数否定有效短定义。这是有限模式检查，不代表能识别所有同义反复或判断知识真伪。

### 最终正文共用审查：持久化与只读都执行

1. 核心一至少交接本步目标概念，另带本步相关词、可用已知前提、仍须解释的词；不接受空词表来跳过审查。只有来源中出现过名称或用户说“学过”，不能放进已验证前提；未知仍为未知。
2. 核心二逐项写最小定义和当前教学稿。目标/开场可先报名称，但同一单元也要落地；开场只点题，不能先用未定义词推理。所有用户将看到的解释、例子、任务、反馈、下一步及图示文字等价说明都纳入检查。
3. 持久化新签发自动调用 `teaching_review.review_teaching_content`，词表使用签发时重读的简报，已知前提只取已校验的 `teaching_basis.anchor_ids`。缺定义、定义含未先落地的登记依赖或明显坏定义时，在写入前拒绝。新增词表不得嵌进用户对象；旧教学记录不逆向迁移。
4. 只读/无个人数据分支使用同一函数，CLI 为 `py -3 -B -X utf8 scripts/teaching_review.py`：从标准输入传入 `concept_inventory / verified_concept_ids / required_terms / content`，仅向标准输出回检查报告；不接收 Vault 路径、不建档、不记日志。`content` 是实际用户教学字段，不带 teaching_basis；[合成示例](../templates/teaching-review-session.json)可用于试跑，不能当作真人画像。词表和已知前提仍必须来自本轮允许的材料与证据；程序不能证明调用者自填的知识状态是真的。
5. 程序通过只返回 `structural_pass + semantic_review_required`。Agent 必须再从最终文字倒查：有没有词表外的新承重概念、有没有用另一个陌生词作定义、有没有只报“用来排序”却没说排序对象及意义、有没有在用户未理解前给出推导。发现问题就补最小定义或退回补前置，不再堆新术语。检查后的稿件有任何改动，重新检查实际输出；不得沿用旧通过结果。

执行上，Agent 先审草稿再调用 `issue-teaching`，该函数内部只有自动校验，不会中途等待语义确认；函数返回后再核对最终投影，尤其是适配器覆盖的反馈/下一步。若最终稿不合格，不展示，修正后重新签发，保留旧记录但不伪造已作答。只读分支没有签发环节，纯内存检查及人工审阅通过后直接输出本步。

本检查按登记名称/别名字面匹配，统一大小写、全半角和空白，ASCII 用标识符边界，重叠名称优先匹配较长者；歧义名称拒绝自动归属。它不自动分词、不发现所有未登记概念、不读取图片资产、不替代事实核验、未见题防泄漏门或掌握检验。图内标签由 Agent 另审。宿主不能执行脚本时按同一清单人工审查，明确“未运行程序检查”，不能伪称已通过。

### 新模型：先意义，后公式

当当前学习对象是模型、算法、符号或图示，先讲**它解决什么问题 → 每个对象/高低/箭头表示什么 → 一个最小例子**，再问一个需要用户自己说明含义的小问题并等待。根据回答决定继续、换说法或补前置；只有必要且含义已衔接时才给公式或复杂推导。熟练者已有同范围理解证据时不机械重复；简单概念也不为凑流程另开文件。

例如用户问聚焦圆锥的意义，先说明“它是 Agent 比较剩余可学步骤先后顺序的内部表示；高低代表更值得先试，不等于已经掌握；圆锥形状本身不参与计算”。随后用两条已符合学习条件、先前路径/成本检查仍分不出先后的步骤作例子，问用户为什么“位置高”不等于“已经会”。不先抛 Focus 公式、Pareto 等未解释名词。

在交接和会话中区分三种事实：**提过名称**只能说明接触过；**已说明含义**只能减少机械重复，仍须问清不懂处；**有当前范围的理解表现**才支持本步继续，完整掌握仍按原合同验证。不要新增一个“讲过=mastered”的捷径。用户明确“不懂含义”时立即暂停相关推理；这条反馈交给核心一保留/修正缺口，也约束核心二下一次讲法。无写权限时仅用于本会话，不能暗存新画像。
